...
VALUES
('RS102', 'IS121', '2023-06-07'),
('RS103', 'IS122', '2023-08-07'),
...
UPDATE members
SET member_address='124 Main St' WHERE member_id='C101'

--Task 3: Delete a Record from the Issued Status Table 
-- Objective: Delete the record with issued_id = 'IS131' from the issued_status table.
DELETE FROM issued_status WHERE issued_id = 'IS131';
...
SELECT issued_member_id,COUNT(issued_id) FROM issued_status GROUP BY 1 HAVING COUNT(issued_id)>1

--Task 6: Create Summary Tables: Used CTAS to generate new tables based on query results - 
--each book and total book_issued_cnt**
CREATE TABLE total_book_issued_cnt AS SELECT 
b.isbn,b.book_title,count(ist.issued_book_isbn) AS issued_count FROM
issued_status as ist JOIN books as b ON ist.issued_book_isbn=b.isbn GROUP BY 1,2;
...
JOIN books as b ON ist.issued_book_isbn=b.isbn GROUP BY 1;
...
ON e1.branch_id = b.branch_id JOIN employees as e2
ON e2.emp_id = b.manager_id;

-- task 11 - Create a Table of Books with Rental Price Above a Certain Threshold:
CREATE TABLE expensive_books AS SELECT * FROM books
...
CURRENT_DATE - ist.issued_date as over_dues_days FROM issued_status as ist JOIN members as m
ON m.member_id = ist.issued_member_id JOIN books as bk ON bk.isbn = ist.issued_book_isbn LEFT JOIN 
return_status as rs ON rs.issued_id = ist.issued_id WHERE rs.return_date IS NULL AND
...
JOIN employees as e ON e.emp_id = ist.issued_emp_id JOIN branch as b ON e.branch_id = b.branch_id
GROUP BY 1, 


--Task 17: Stored Procedure Objective: Create a stored procedure to manage the status of books in a library system. 
...
LANGUAGE plpgsql AS $$ DECLARE v_status VARCHAR(10);
BEGIN
-- all the code
